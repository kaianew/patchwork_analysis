...
# FIXME: random effect structure is non-estimable
ttff_patch_model = glm(correct ~ ttff_patch + condition + bug,
                 data = gaze_valid_had_patch, family = binomial)
summary(ttff_patch_model)

# FIXME: should we include condition for the subsetted df?
ttff_patch_cox = coxme(Surv(time_minutes, status) ~ condition + ttff_patch + (1 | PID) + (1 | bug), data = gaze_valid_had_patch)
...
attention_switch_cox = coxme(Surv(time_minutes, status) ~ condition + attention_switching_rate + (1 | PID) + (1 | bug), data = gaze_valid_df)
summary(attention_switch_cox)
# after i recalculated attention switching to only include relevant AOIs, significant effect disappeared
...
for (col in relevant_aoi_prop_cols) {
  ...
  p_values[col] = coef_table[col, "Pr(>|t|)"]
...
# FIXME: again, do survival analysis
p_values_time = c()
for (col in relevant_aoi_prop_cols) {
  ...
  # save p-value for multcomp :3c
  coef_table = summary(model)$coefficients
  p_values_time[col] = coef_table[col, "Pr(>|t|)"]
}
