...
# P2 was additionally censored (cut off at 20 min)
full_df$status[full_df$PID == 'P2' & full_df$task_no == 3] = FALSE
```
...
# Prep fix_site_same col to see if they used the given patch unchanged
full_df_had_patch$fix_site_same = ifelse(full_df_had_patch$fix_site_same == 'N', FALSE, ifelse(full_df_had_patch$fix_site_same == 'Y', TRUE, NA))
full_df_had_patch$changed_patch = !full_df_had_patch$fix_site_same
...
# additionally check whether they were cut off too early (just P2 t3) to see if that meaningfully contributes to the model
...
# First determine if cutting someone off early was significant for correctness (survival analysis handles this automatically, but need to check fro correctness)

full_correctness_model_w_cutoff = glmer(correct ~ condition + professional_YOE
                              + java_intellij_experience + self_efficacy + num_prev + cut_off
                              + (1 | PID) + (1 | bug),
                              data = full_df,
                              family = binomial)
...
full_correctness_model = glmer(correct ~ condition + professional_YOE
                              ...
                              data = full_df,
                              family = binomial)
cutoff_LRT = anova(full_correctness_model_w_cutoff, full_correctness_model)
cutoff_LRT
# answer: no. now we test meaningful predictors

# Do LRT to see whether condition or professional_YOE improve the model
...
correct_matrix = as.data.frame.matrix(correct_tab)
patch_correct_matrix = as.data.frame.matrix(patch_correct_tab)
submit_correct_matrix = as.data.frame.matrix(submit_correct_tab)

correct_matrix
sum(correct_matrix)
patch_correct_matrix
sum(patch_correct_matrix)
submit_correct_matrix
sum(submit_correct_matrix)
...
# Select two columns and save to CSV
new_df <- gaze_valid_df[, c("PID", "task_no")]

write.csv(new_df, "valid_ptasks.csv", row.names = FALSE)

P19t2 = gaze_valid_df[gaze_valid_df$PID == "P19" & gaze_valid_df$task_no == 2, ]
P19t2

```

# RQ1 - Signals of Developer Judgments
```{r}
# Question: Did people use the patch they were given more often in either condition?
# find participants from full_df who evaluated the patch
# fix_site_same - syntactically same at fix site (did not change the patch)

applied_unchanged_df = full_df_had_patch
# Note: P6 also mentioned that they didn't look at the patch in their t1
applied_unchanged_df = applied_unchanged_df[!(applied_unchanged_df$PID == "P6" & applied_unchanged_df$task_no == 1), ]
# make OR make more sense by inverting condition we compare to for this particular model
# applied_unchanged_df$condition = relevel(factor(applied_unchanged_df$condition), ref = "overfitting")

# in 4 participant-tasks, they did not evaluate the patch 
applied_unchanged_model = glmer(changed_patch ~ condition + (1 | PID) + (1 | bug),
                              data = applied_unchanged_df,
                              family = binomial)
summary(applied_unchanged_model)

# find odds ratio:
exp(fixef(applied_unchanged_model))

table(applied_unchanged_df$condition, applied_unchanged_df$changed_patch)
```
...
# find odds ratio, i think it's not very interpretable in this case, but we check
exp(fixef(patch_dur_model))
