...
# paths = ['/home/kaia/patchwork_aggregation/P34_t2_fixation_filtered.csv',
#          '/home/kaia/patchwork_aggregation/P34_t3_fixation_filtered.csv']
...
# also do the same for fixation_sequences and lengths (for HMM training)
fixation_sequences_correct = np.empty((0, 1), dtype=int)
sequence_lengths_correct = []
fixation_sequences_incorrect = np.empty((0, 1), dtype=int)
sequence_lengths_incorrect = []
fixation_sequences_no_patch = np.empty((0, 1), dtype=int)
sequence_lengths_no_patch = []
fixation_sequences_patch = np.empty((0, 1), dtype=int)
sequence_lengths_patch = []
...
# need to integer-encode the AOIs for HMM training, so create a mapping of AOI to integer
integer_encoding = {}
current_integer = 0
...
def aggregate(full_counts, counts, 
    ...
    full_sequences = np.concatenate((full_sequences, sequence), axis=0)
    decoded_full_sequences.append(decoded_sequence.flatten().tolist())
    full_sequence_lengths.append(len(sequence))

    return full_counts, full_durations, full_sequences, full_sequence_lengths, decoded_full_sequences
...
for path in paths:
    ...
    # Used in Markov chain and HMM training, as well as attention switching metric
    aoi_sequence = []
    ...
    for fixation_id in fixation_ids:
        ...
        if aoi == '-' or aoi == 'OOB':
            ...
            if aoi not in integer_encoding:
                integer_encoding[aoi] = current_integer
                current_integer += 1
            aoi_sequence.append(integer_encoding[aoi])
            decoded_aoi_sequence.append(aoi)

    # Now aoi_sequence is a single list of integer-encoded AOIs for this task
    # decoded_aoi_sequence is the corresponding list of original AOI labels
    aoi_sequence_array = np.array(aoi_sequence).reshape(-1, 1)
    decoded_aoi_sequence_array = np.array(decoded_aoi_sequence).reshape(-1, 1)
    ...
    if got_correct:
        ...
        fixation_sequences_correct, sequence_lengths_correct, decoded_fixation_sequences_correct) = \
        aggregate(fixation_counts_correct, fixation_counts, fixation_durations_correct, 
                      fixation_durations, fixation_sequences_correct, aoi_sequence_array, 
                      sequence_lengths_correct, decoded_fixation_sequences_correct, decoded_aoi_sequence_array)
    else:
        (fixation_counts_incorrect, fixation_durations_incorrect,
        fixation_sequences_incorrect, sequence_lengths_incorrect, decoded_fixation_sequences_incorrect) = \
        aggregate(fixation_counts_incorrect, fixation_counts, fixation_durations_incorrect, 
                      fixation_durations, 
                      fixation_sequences_incorrect, aoi_sequence_array, 
                      sequence_lengths_incorrect, decoded_fixation_sequences_incorrect, decoded_aoi_sequence_array)
    
    if has_patch:
        (fixation_counts_patch, fixation_durations_patch,
        fixation_sequences_patch, sequence_lengths_patch, decoded_fixation_sequences_patch) = \
        aggregate(fixation_counts_patch, fixation_counts, fixation_durations_patch, 
                      fixation_durations, 
                      fixation_sequences_patch, aoi_sequence_array, 
                      sequence_lengths_patch, decoded_fixation_sequences_patch, decoded_aoi_sequence_array)
    else:
        (fixation_counts_no_patch, fixation_durations_no_patch,
        fixation_sequences_no_patch, sequence_lengths_no_patch, decoded_fixation_sequences_no_patch) = \
        aggregate(fixation_counts_no_patch, fixation_counts, fixation_durations_no_patch, 
                      fixation_durations, 
                      fixation_sequences_no_patch, aoi_sequence_array, 
                      sequence_lengths_no_patch, decoded_fixation_sequences_no_patch, decoded_aoi_sequence_array)
...
# ensure integer datatype in sequences for HMM training
fixation_sequences_correct = fixation_sequences_correct.astype(int)
fixation_sequences_incorrect = fixation_sequences_incorrect.astype(int)
fixation_sequences_patch = fixation_sequences_patch.astype(int)
fixation_sequences_no_patch = fixation_sequences_no_patch.astype(int)

# train hmms for incorrect vs. correct, patch vs. no patch (draw k vs. BIC). note: we want categorical hmms
best_bic_model_correct, best_aic_model_correct, \
best_bic_k_correct, best_aic_k_correct = train_hmm(fixation_sequences_correct, sequence_lengths_correct, int(np.max(fixation_sequences_correct) + 1), 'Correct')
best_bic_model_incorrect, best_aic_model_incorrect, \
best_bic_k_incorrect, best_aic_k_incorrect = train_hmm(fixation_sequences_incorrect, sequence_lengths_incorrect, int(np.max(fixation_sequences_incorrect) + 1), 'Incorrect')
best_bic_model_patch, best_aic_model_patch, \
best_bic_k_patch, best_aic_k_patch = train_hmm(fixation_sequences_patch, sequence_lengths_patch, int(np.max(fixation_sequences_patch) + 1), 'Patch')
best_bic_model_no_patch, best_aic_model_no_patch, \
best_bic_k_no_patch, best_aic_k_no_patch = train_hmm(fixation_sequences_no_patch, sequence_lengths_no_patch, int(np.max(fixation_sequences_no_patch) + 1), 'No Patch')

# print best k values here; will also want to visualize this with a plot of k vs. BIC and k vs. AIC
print(f"Best k values - Correct: BIC={best_bic_k_correct}, AIC={best_aic_k_correct}")
print(f"Best k values - Incorrect: BIC={best_bic_k_incorrect}, AIC={best_aic_k_incorrect}")
print(f"Best k values - Patch: BIC={best_bic_k_patch}, AIC={best_aic_k_patch}")
print(f"Best k values - No Patch: BIC={best_bic_k_no_patch}, AIC={best_aic_k_no_patch}")

# print integer encoding for this run so we can interpret the HMM states
print("Integer encoding of AOIs:")
for item in integer_encoding.items():
    print(item)

# pickle hmms, we can figure out what to do about them later 
with open('hmm_models.pkl', 'wb') as f:
    pkl.dump({
        'best_bic_model_correct': best_bic_model_correct,
        'best_aic_model_correct': best_aic_model_correct,
        'best_bic_k_correct': best_bic_k_correct,
        'best_aic_k_correct': best_aic_k_correct,
        'best_bic_model_incorrect': best_bic_model_incorrect,
        'best_aic_model_incorrect': best_aic_model_incorrect,
        'best_bic_k_incorrect': best_bic_k_incorrect,
        'best_aic_k_incorrect': best_aic_k_incorrect,
        'best_bic_model_patch': best_bic_model_patch,
        'best_aic_model_patch': best_aic_model_patch,
        'best_bic_k_patch': best_bic_k_patch,
        'best_aic_k_patch': best_aic_k_patch,
        'best_bic_model_no_patch': best_bic_model_no_patch,
        'best_aic_model_no_patch': best_aic_model_no_patch,
        'best_bic_k_no_patch': best_bic_k_no_patch,
        'best_aic_k_no_patch': best_aic_k_no_patch
    }, f)

# pickle sequences and lengths, might want to use one model to predict another to see if they’re much different
with open('hmm_sequences.pkl', 'wb') as f:
    pkl.dump({
        'fixation_sequences_correct': fixation_sequences_correct,
        'sequence_lengths_correct': sequence_lengths_correct,
        'fixation_sequences_incorrect': fixation_sequences_incorrect,
        'sequence_lengths_incorrect': sequence_lengths_incorrect,
        'fixation_sequences_patch': fixation_sequences_patch,
        'sequence_lengths_patch': sequence_lengths_patch,
        'fixation_sequences_no_patch': fixation_sequences_no_patch,
        'sequence_lengths_no_patch': sequence_lengths_no_patch
    }, f)
